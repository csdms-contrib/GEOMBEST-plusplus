...
while flag == 1
    ...
    if exist (filename) == 2 
        ...
        for row = 1:T                
                ...
                runfiles(M).overwashflux(row) = numbers(row + 2,7); % DW - volume of overwash deposited onto te backbarrier from storms
                ...
                runfiles(M).shearcrit(row) = numbers(row + 2,10); % RL - critical shear stress
